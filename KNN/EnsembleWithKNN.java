...
public class EnsembleWithKNN{
	public static void main(String[] args) throws Exception {
        ...
        // remove un necessarry attributes
        Remove r = new Remove();
	...
	private static void evaluateKNN(Instances train, Instances test){
		try {
			// class for KNN classifier in weka
			Classifier ibk = new IBk(10);
			//performCV(train,ibk); //  to find k value
			// implementing bagging with KNN
			Bagging bagger = new Bagging();
			bagger.buildClassifier(train);
			bagger.setClassifier(ibk);
			// Evaluating performance of KNN with bagging 
			Evaluation eval = new Evaluation(test);
			eval.evaluateModel(bagger, test);
			System.out.println();
			System.out.println("Metrics from KNN");
    ...
    private static void classStatistics(Instances data) {
        AttributeStats status = data.attributeStats(data.classIndex());
        System.out.println("Number of records with 0 class = " + status.nominalCounts[0]);
        System.out.println("Number of records with 1 class = " + status.nominalCounts[1]);
    }
    ...
    private static void evaluateForest(Instances data) throws Exception {
        ...
        // implementing KNN classifier on the resampled data
        evaluateKNN(train,test);
    }

    private static void performCV(Instances data, Classifier r) throws Exception {
    	System.out.println("in perform cv");
        int folds = 10;
        ...
        double total0 = 0;
        double total1 = 0;
        double acc = 0,prec = 0,rec = 0,kap = 0;
        for (int n = 0; n < folds; n++) {
            ...
            total0 += eval.precision(0);
            total1 += eval.precision(1);
            acc = eval.weightedFMeasure();
            prec = eval.weightedPrecision();
            rec = eval.weightedRecall();
            kap = eval.kappa();
        }
        //System.out.println(total0 / 10);
        //System.out.println(total1 / 10);
        System.out.println("accuracy "+ acc/10);
        System.out.println("precision "+ prec/10);
        System.out.println("recal "+ rec/10);
        System.out.println("kap" + kap/10);
    }

    private static void write(Instances dataSet) throws Exception {
        ArffSaver saver = new ArffSaver();
        saver.setInstances(dataSet);
        saver.setFile(new File("data.arff"));
        saver.setDestination(new File("data.arff"));
        saver.writeBatch();
    }
}
